...
if (isset($_POST['submit'])) {
  ...
  if (!empty($user) && !empty($message)) {
    $sql = "INSERT INTO messages_bub (user, message) VALUES ('$user', '$message')";
...
$sql = "SELECT * FROM messages_bub ORDER BY create_at ASC";
...
<body>
    ...
    <div class="smrad_wrapper">
      <svg class="svg" viewbox="0 0 1821.3333 1024">
        <foreignObject x="800.32332" y="500.8043" width="280" height="200">
          <form method="POST" action="includes/kopanie.php">
            <input type='hidden' name='catch_govno' value='1'>
            <div class="res">
            <p class="result" id="result"></p>
              ...
              <button class="button" id="chop_tree_button" type="submit" name="catch_govno">копать</button>
            </div>
          </form>
        </foreignObject>
      </svg>
  ...
  <img id="delph" class="delph" src="/jpg/delph.png" alt="">
  <img id="map" src="/jpg/yama.png" alt="Смрадово">
  </div>

  <script>
document.getElementById("delph").addEventListener("click", getJoke);

function getJoke() {
  fetch('includes/anek.php')
  .then(response => response.text())
    .then(joke => {
      alert(joke); // вывод анекдота в виде алерта
    })
  ...
  <script>
    ...
    function startTimer() {
      ...
      $.ajax({
        url: "includes/kopanie.php",
        type: "GET",
        dataType: "json",
        success: function(data) {
          $('#result').text(data.message_gov);
        },
...
<table>
    ...
    <?php
    $sql = "SELECT user_id, quantity, item_id FROM user_items WHERE item_id = 2 ORDER BY quantity DESC LIMIT 5";
